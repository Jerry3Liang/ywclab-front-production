<template>
  <h1 style="padding-bottom: 5px">PLR 實驗 Data 比較</h1>
  ...
  <VueDraggable ref="el" v-model="selectedFile" @end="onDragEnd" style="width: 280px;">
...
<!--          </div>-->
  ...
  <canvas id="plr-chart" style="margin-top: 20px;"></canvas>
  <button class="btn btn-outline-success"
          type="button"
...
const onFileChange = async (event) => {
  ...
  selectedEyes.value = Array.from({ length: selectedFile.value.length }, () => ({right: false }));
  ...
  try{
    ...
    const response = await axiosApi.post("/plr/excelDataToLine", formData);
    PLRRawData.value = response.data;
    console.log(PLRRawData.value);
    ...
    if (error.response && Array.isArray(error.response.data)) {
      ...
      await Swal.fire({
        icon: 'error',
        title: '檔案轉換限制',
        html: `<p>因所使用的格式轉換程式為免費版，有檔案轉換限制</p><P>請重新啟動服務！</P>`  //單一錯誤訊息
      });
      window.location.reload();
    } else {
      // 未知錯誤或非預期錯誤的處理
...
const drawChart = (datasets) => {
  const ctx = document.getElementById('plr-chart').getContext('2d');
  if (chartInstance.value) {
    chartInstance.value.destroy();
  }

  let number = [];
  for(let i = 1; i <= PLRRawData.value[0].expRightEyeRawData.length; i++){
    number.push(i);
  }
  console.log(number)
...
watch(
    () => selectedEyes.value,
    () => {
      if (PLRRawData.value.length > 0) {
        isChartDrawing.value = true;  // 繪製前鎖定多選框
        const datasets = [];

        PLRRawData.value.forEach((data, index) => {
...
const onDragEnd = (event) => {
  ...
  const movedOPsRawData = PLRRawData.value.splice(oldIndex, 1)[0];
  PLRRawData.value.splice(newIndex, 0, movedOPsRawData);
};
...
const scrollToChart = () => {
  ...
  isHandScroll.value = false;
  const chartElement = document.getElementById('plr-chart');
  if (chartElement) {
