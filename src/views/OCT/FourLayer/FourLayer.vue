<template>
  <h1 style="padding-bottom: 5px">{{$t('FourLayerTitle')}}</h1>
...
<!--      </select>-->
  ...
  <table class="table" v-if="showTables" style="margin-top: 10px">
    ...
    <tr>
      ...
      <th scope="col">NFL-INL</th>
      <th scope="col">OPL-ONL</th>
      <th scope="col">IS-OS-RPE</th>
    </tr>
    </thead>
    <tbody>
    <tr v-for="data in filteredData">
      <td style="user-select: none">{{data.groupName}}</td>
      <td v-for="ndata in data.octfourLayerDataList" style="user-select: none">{{ndata}}</td>
    </tr>
    </tbody>
  </table>
  <button class="btn btn-outline-success"
...
watch(
    ...
    async () => {
          try{
            ...
            const response = await axiosApi.post("/fourLayer/excelDataToTable", formData);
            octFourLayerData.value = response.data;
            finalDataMap.value = response.data;
            // console.log(octFourLayerData.value);

            const setGroupNames = new Set([]);
            for(let i = 0; i < octFourLayerData.value.length; i++){
              setGroupNames.add(octFourLayerData.value[i].groupName.substring(0, octFourLayerData.value[i].groupName.indexOf('_')));
            }
...
const filteredData = computed(() => {
  ...
  return octFourLayerData.value.filter((data) => {
...
watch(
    ...
    () => {
      ...
      filteredData.value.forEach((data) => {
        result.push({ groupName: data.groupName, data: data.octfourLayerDataList});
      });
...
const downloadCWaveTableData = async () => {
  ...
  let data = {
    // "octFourLayerDataMapSet" : Object.fromEntries(finalMapForBackend.value)
    "octFourLayerDataMapSet" : Object.fromEntries(orderedMap)
  };

  const response = await axiosApi.post("/fourLayer/downloadOCTFourLayerData", data, {
    headers: {
  ...
  // 從回應的 headers 中獲取文件名
  let fileName = formattedDate + '_OCT-Four Layer.xlsx';//預設文件名
  ...
  selectGroupName.value.forEach(groupPrefix => {
    ...
    finalDataMap.value.forEach((data) => {
      if (data.groupName.startsWith(groupPrefix + '_')) {
        orderedMap.set(data.groupName, data.octfourLayerDataList)
      }
    })
  })
...
const copyLeftEyeTable = () => {
  ...
  ).map(data => [data.groupName, ...data.octfourLayerDataList]);
...
const copyTable = () => {
  const tableData = filteredData.value.map(data => [data.groupName, ...data.octfourLayerDataList]);
