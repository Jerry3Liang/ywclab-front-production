<template>
  <h1 style="padding-bottom: 5px">PhNR 實驗 Data 分析</h1>
  <div class="input-group" style="width: 500px">
    ...
    <select class="form-select" style="width: 130px" v-if="selectedFile && loading === false" v-model="selectedGroup">
      <option value="null" selected>請選擇組別</option>
  ...
  <table class="table" v-if="showTables" style="margin-top: 10px">
    <thead>
    <tr>
      <th scope="col">老鼠編號</th>
      <th scope="col">Eye (右眼, P1N1(µV))</th>
      <th scope="col">ms</th>
      <th scope="col">Eye (右眼, N1P2(µV))</th>
      <th scope="col">ms</th>
      <th scope="col">Eye (左眼, P1N1(µV))</th>
      <th scope="col">ms</th>
      <th scope="col">Eye (左眼, N1P2(µV))</th>
      <th scope="col">ms</th>
    </tr>
    </thead>
    <tbody>
  ...
  <button class="btn btn-outline-success"
          ...
          v-if="showTables">複製左眼 P1N1</button>
  <button class="btn btn-outline-success"
          type="button"
          @click="copyTableLeftEyeN1P2Values"
          v-if="showTables"
          style="margin-left: 10px">複製左眼 N1P2</button>
  <button class="btn btn-outline-success"
          type="button"
          @click="copyTableRightEyeP1N1Values"
          v-if="showTables"
          style="margin-left: 10px">複製右眼 P1N1</button>
  <button class="btn btn-outline-success"
          type="button"
          @click="copyTableRightEyeN1P2Values"
          v-if="showTables"
          style="margin-left: 10px">複製右眼 N1P2</button>
  <button class="btn btn-outline-success"
          type="button"
          @click="copyTableAllEyeP1N1Values"
          v-if="showTables"
          style="margin-left: 10px">複製全部 P1N1</button>
  <button class="btn btn-outline-success"
          type="button"
          @click="copyTableAllEyeN1P2Values"
          v-if="showTables"
          style="margin-left: 10px">複製全部 N1P2</button>
  <button class="btn btn-outline-success"
          @click="scrollToTop"
          style="position: fixed; bottom: 20px; right: 20px;"
          v-if="showTables">回到最上面</button>
</template>
...
watch(
    ...
    async () => {
      try{
        ...
        const response = await axiosApi.post("/phnr/excelDataToTable", formData);
        PhNRRawData.value = response.data;
        console.log(PhNRRawData.value);

        const setGroupNames = new Set([]);
        for(let i = 0; i < PhNRRawData.value.length; i++){
          setGroupNames.add(PhNRRawData.value[i].groupName.substring(0, PhNRRawData.value[i].groupName.indexOf('_')));
        }
...
const filteredData = computed(() => {
  ...
  return PhNRRawData.value.filter((data) => {
