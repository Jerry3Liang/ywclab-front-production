<template>
  <h1 style="padding-bottom: 5px">PhNR 實驗 Data 波形比較</h1>
  ...
  <canvas id="vep-chart" style="margin-top: 20px;"></canvas>
  <button class="btn btn-outline-success"
          type="button"
...
const onFileChange = async (event) => {
  ...
  try{
    ...
    const response = await axiosApi.post("/phnr/excelDataToLine", formData);
...
const drawChart = (datasets) => {
  const ctx = document.getElementById('vep-chart').getContext('2d');
  ...
  chartInstance.value = new Chart(ctx, {
    type: 'line',
    data: {
      labels: PhNRRawData.value[0].dataMilliSec, // 假設所有檔案的 X 軸標籤相同
      datasets: datasets,
    },
    options: {
      responsive: true,
...
watch(
    () => selectedEyes.value,
    () => {
      if (PhNRRawData.value.length > 0) {
        isChartDrawing.value = true;  // 繪製前鎖定多選框
        const datasets = [];

        PhNRRawData.value.forEach((data, index) => {
...
const onDragEnd = (event) => {
  ...
  const movedOPsRawData = PhNRRawData.value.splice(oldIndex, 1)[0];
  PhNRRawData.value.splice(newIndex, 0, movedOPsRawData);
};
