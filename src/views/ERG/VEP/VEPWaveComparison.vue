<template>
  <h1 style="padding-bottom: 5px">VEP 實驗 Data 波形比較</h1>
...
const onFileChange = async (event) => {
  ...
  try{
    ...
    const response = await axiosApi.post("/vep/excelDataToLine", formData);
...
const drawChart = (datasets) => {
  ...
  chartInstance.value = new Chart(ctx, {
    type: 'line',
    data: {
      labels: VEPRawData.value[0].dataMilliSec, // 假設所有檔案的 X 軸標籤相同
      datasets: datasets,
    },
    options: {
      responsive: true,
...
watch(
    () => selectedEyes.value,
    () => {
      if (VEPRawData.value.length > 0) {
        isChartDrawing.value = true;  // 繪製前鎖定多選框
        const datasets = [];

        VEPRawData.value.forEach((data, index) => {
...
const onDragEnd = (event) => {
  ...
  const movedOPsRawData = VEPRawData.value.splice(oldIndex, 1)[0];
  VEPRawData.value.splice(newIndex, 0, movedOPsRawData);
};
